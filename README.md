...
> **R: Esto ocurre debido a que el preprocesador reemplaza las directivas, los #include ahora son cientos de lineas con declaraciones de funciones, provenientes de los headers, como printf lo que alarga el código.> ** 
...
> **R:Esto ocurre debido a que una de las tareas del preprocesador es de borrar los cometarios incluidos en el código, ya que no son necesarios para compilar un archivo.**
...
> **R: $ grep -n "CUADRADO" programa.i --> 2246:    printf("CUADRADO(%d)      = %d\n", 5, ((5) * (5)));**
...
> **R: $ gcc -E programa.c | grep "Iniciando" --> no devuelve nada /// $ gcc -E -DDEBUG programa.c | grep "Iniciando" --> printf("[DEBUG] %s\n", ("Iniciando main"));**
...
> **R: Las lineas indican al compilador de que linea y de que archivo proviene el código que sigue. La declaracion `printf` proviene del archivo C:/msys64/ucrt64/include/stdio.h .> **
...
> **R: 1° Linea --> guarda el valor actual del registro rbp en la pila, para no perder el contexto de la función que llamó a sumar 
2° Linea --> directiva para el manejo de excepciones en Windows, le avisa al sistema que se guardó rbp
3° Linea --> copia el valor actual del rsp al rbp, estableciendo un punto de referencia fijo para toda la función.
4° Linea --> directiva de Windows, registra que rbp es el frame pointer
> **
...
0000000000000000 r .xdata
                 ...
                 U area_circulo
                 U factorial
0000000000000158 T imprimir_separador
...
> **R: Esto ocurre porque en programa.c solo existe la declaración de area_circulo (copiada del header por el preprocesador) y por esto se le asigna la U, mientras que en matematica.c esta la definicion y por eso se le asigna la T**
...
00000001400015e0 T area_circulo**
...
> **R: Quedan porque son funciones de la biblioteca dinámica del sistema, luego en la ejecucion del programa las resuelve el cargador dinámico **
...
> **R: La diferencia es que la macro "desaparece" en el preprocesamiento y no tiene verificación de tipos, mienmtras que una funcion real es compilada y queda en el código del ejecutable y si tiene verificación de tipos.**
...
> **R: Un simbolo T representa código ejecutable (funciones) y vive en la sección .text, mientras que un simbolo D representa una variable global con valor inicial y vive en la sección .data.**
...
> **R: $ ldd programa.exe -->
        ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7ffbb4e00000)
        KERNEL32.DLL => /c/WINDOWS/System32/KERNEL32.DLL (0x7ffbb3c20000)
        aswhook.dll => /c/Program Files/Avast Software/Avast/aswhook.dll (0x7ffb9c090000)
        KERNELBASE.dll => /c/WINDOWS/System32/KERNELBASE.dll (0x7ffbb1a80000)
        ucrtbase.dll => /c/WINDOWS/System32/ucrtbase.dll (0x7ffbb17b0000)
**

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R: Esto no es necesario debido a que el gcc incluye automaticamente la biblioteca estandar de C al enlazar, sin necesidad de especificarla**
